...
from sqlalchemy import  select, func, desc , or_ , and_ ,Column,Integer
...
@app.get('/MejorPiloto')
def get_MejorPiloto():
    return conn.execute(select(func.sum(t_results.c.points).label('Puntos'),(t_drivers.c.driverRef).label('PilotoF1'))\
